...
3. After that, there should be a .sav file in `AppData\Local\MarblesOnStream\Saved\SaveGames\Sessions` with the name of the session that you made.
...
7. Set `%converter%` argument, in both Initializer and PointAlloc, to the full path of GvasConverter.exe file, which was extracted from the [Converter.zip](https://github.com/SyrDim/MarblesOnStreamLeaderboardParser/releases) that you downloaded. (Make sure that it's not surrounded by "")
